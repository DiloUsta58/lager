...
const PROTECTED_FIELDS = ["material", "enummer"];
...
/* =========================
   EDIT-KONFIGURATION
========================= */
const LOGOUT_TIMEOUT_MS = 6 * 60 * 1000; // 6 Minuten
const EDIT_TIMEOUT_MS = 3 * 60 * 1000; // 8 Minute
const EDIT_KEY_HASH =
...
function startLogoutWatcher() {
  if (!loggedIn) return;   // 🔥 FEHLT AKTUELL

  stopLogoutWatcher();

  logoutTimer = setInterval(() => {
    if (!loggedIn) return; // 🔥 zusätzlicher Schutz
...
/* ==========================
   COLLAPSIBLE COLUMN-TOGGLES
   Fügt einen Toggle-Button vor jeder .column-toggle-grid ein
   und verwaltet den collapsed-state via CSS.
========================== */
function attachColumnToggleCollapse() {
  document.querySelectorAll('.column-toggle-grid').forEach(grid => {
    const parent = grid.parentElement;
    if (!parent) return;

    // avoid duplicate buttons
    if (parent.querySelector('.col-toggle-btn')) return;

    // create wrapper to avoid mutating surrounding layout
    const wrapper = document.createElement('div');
    wrapper.className = 'column-toggle-container';
    ...
    btn.textContent = 'Spalten Aufklappen ▶';

    btn.addEventListener('click', () => {
      const collapsed = wrapper.classList.toggle('collapsed');
      btn.textContent = collapsed ? 'Spalten Aufklappen ▶' : 'Spalten Zuklappen ▲';
    });

    // insert wrapper and move grid inside it
